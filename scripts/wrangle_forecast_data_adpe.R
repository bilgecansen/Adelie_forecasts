...
for (i in 1:50) {
  colnames(env_mat_fore[[i]]) <- 1900:2100
  rownames(env_mat_fore[[i]]) <- sites_coupled
}

saveRDS(env_mat_fore_avg, "data/data_coupled_transformed_avg_adpe.rds")
saveRDS(env_mat_fore, "data/data_coupled_transformed_raw_adpe.rds")
...
env_mat_fore_std <- foreach(i = 1:50) %do%
  {
    (env_mat_fore[[i]] - x_mean) / x_sd
  }

saveRDS(env_mat_fore_avg_std, "data/data_coupled_normalized_avg_adpe.rds")
saveRDS(env_mat_fore_std, "data/data_coupled_normalized_raw_adpe.rds")
